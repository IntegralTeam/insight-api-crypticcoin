...
var bitcore = require('bitcore-lib-crypticcoin');
...
BlockController.prototype.getBlockReward = function(blockIndex) {
  var coin = new BN(1e8);
  var subsidy = (new BN(1655)).mul(coin);
  var premineSubsidy = (new BN(4179234070)).mul(coin);
  var nSubsidyHalvingInterval = 10483200;

  if (blockIndex == 1) {
    return premineSubsidy;
  }else if (blockIndex >= 33000){
    subsidy = subsidy.div(new BN(10));
  }

  var halvings = Math.floor((blockIndex + 1) / nSubsidyHalvingInterval);
  // Force block reward to zero when right shift is undefined.
  if (halvings >= 11) {
    return 0;
  }

  // Subsidy is cut in half every 1048320 blocks
