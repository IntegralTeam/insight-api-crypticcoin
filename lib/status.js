...
StatusController.prototype.show = function(req, res) {
  ...
  case 'getBestBlockHash':
    ...
    break;
  case 'getMasternodes':
    this.getMasternodes(function(err, result) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      res.jsonp(result);
...
StatusController.prototype.getMasternodes = function(callback) {
  this.node.services.bitcoind.listMasternodes(function(err, mnlist) {
    function isActivated(mn) {
	      return mn.status == "activated";
    }
    if (err) {
      return callback(err);
    }
    callback(null, {
      activatedCount: mnlist.filter(isActivated).length,
      totalCount: mnlist.length
    });
  });
};
